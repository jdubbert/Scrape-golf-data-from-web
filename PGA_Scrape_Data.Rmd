...
## Variables
Each of the below variables is created for the years 2010:2018. (ie. top10_2018, top10_2017, etc.)
```{r}
...
16. `masters_appear`
```{r}
print("Total number of Masters Appearances")
```
...
##Get Ranking `ranking`
```{r}
rank_list <- lapply(2010:2018, get_all_data, stat="127")
...
```{r}
top10_list <- lapply(2010:2018, get_all_data, stat="138")
...
```{r}
sa_list <- lapply(2010:2018, get_all_data, stat="120")
...
```{r}
sg_list <- lapply(2010:2018, get_all_data, stat="02675")
...
##`sg_t2g`
```{r}
sg_t2g_list <- lapply(2010:2018, get_all_data, stat="02674")
...
##`sg_putt`
```{r}
sg_putt_list <- lapply(2010:2018, get_all_data, stat="02564")
sg_putt_indx <- lengths(sg_putt_list)
one <- names(sg_putt_list[[1]])
names(sg_putt_list[[1]]) <- c(one[1:8], "TOTAL.SG.PUTTING", one[10])
...
```{r}
d_list <- lapply(2010:2018, get_all_data, stat="101")
...
```{r}
gir_list <- lapply(2010:2018, get_all_data, stat="103")
...
```{r}
putts_list <- lapply(2010:2018, get_all_data, stat="119")
...
```{r}
five_list <- lapply(2010:2018, get_all_data, stat="144")
...
```{r}
bb_list <- lapply(2010:2018, get_all_data, stat="160")
...
```{r}
da_list <- lapply(2010:2018, get_all_data, stat="102")
...
```{r}
p_list <- lapply(2010:2018, get_all_data, stat="374")
...
```{r}
s_list <- lapply(2010:2018, get_all_data, stat="130")
...
## Get number of Masters Apperances
```{r}
m1 <- rep(NA)
player <- c("Matt Kuchar", "Retief Goosen")
names(player)[names(player)=="player_name"] <- "player"
m1 <- cbind(m1, player)
for (i in player) {
  masters_count <-masters_all %>% filter(player==i) %>% count(count)
  print(masters_count)
}
```
...
## Merge all stats
```{r}
ab <- merge(merge(ranking_all, top10_all, by = c("player_name", "year"), all = TRUE), score_avg_all, by = c("player_name", "year"), all = TRUE)
abc <- merge(all_ab, bb_all, by = c("player_name", "year"), all=TRUE)
abcd <- merge(abc, da_all, by = c("player_name", "year"), all=TRUE)
abcde <- merge(abcd, driving_distance_all, by = c("player_name", "year"), all=TRUE)
abcdef <- merge(abcde, five_all, by = c("player_name", "year"), all=TRUE)
abcdefg <- merge(abcdef, gir_all, by = c("player_name", "year"), all=TRUE)
abcdefgh <- merge(abcdefg, p_all, by = c("player_name", "year"), all=TRUE)
abcdefghi <- merge(abcdefgh, putts_all, by = c("player_name", "year"), all=TRUE)
abcdefghij <- merge(abcdefghi, s_all, by = c("player_name", "year"), all=TRUE)
abcdefghijk <- merge(abcdefghij, sg_putt_all, by = c("player_name", "year"), all=TRUE)
abcdefghijkl <- merge(abcdefghijk, sg_t2g_all, by = c("player_name", "year"), all=TRUE)
all_data <- merge(abcdefghijkl, tstrokes_gained_all, by = c("player_name", "year"), all=TRUE)
